    ...
    pub async fn init(db_path: &str, token: Option<String>, target_company: Option<String>) -> Result<Self, String> {
        ...
        let mut response = with_database_timeout("state hydration", async {
            if let Some(ref target) = target_company {
                ...
                db.query(
                    "SELECT record::id(id) AS id, name, role, IF parent_id != null THEN record::id(parent_id) ELSE null END AS parent_id, system_prompt, tools, telemetry, scheduled_tasks, pending_messages, issue_triggers FROM agent WHERE company_id = type::record($target) OR company_id = null;
                ...
                db.query(
                    "SELECT record::id(id) AS id, name, role, IF parent_id != null THEN record::id(parent_id) ELSE null END AS parent_id, system_prompt, tools, telemetry, scheduled_tasks, pending_messages, issue_triggers FROM agent;
